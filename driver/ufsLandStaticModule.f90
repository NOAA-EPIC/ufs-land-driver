...
contains   
  ...
  if(namelist%subset_begin > this%nlocations .or. namelist%subset_end > this%nlocations) &
    stop "subset_begin or location_end not consistent with nlocations in static read"
  ...
  status = nf90_get_var(ncid, varid, this%latitude, &
       start = (/namelist%subset_begin/), count = (/namelist%subset_length/))
   if(status /= nf90_noerr) call handle_err(status)
  ...
  status = nf90_get_var(ncid, varid, this%gvf_monthly, &
       start = (/namelist%subset_begin,1/), count = (/namelist%subset_length,12/))
   if(status /= nf90_noerr) call handle_err(status)
